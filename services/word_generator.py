...
def _add_ai_memo_section(doc, ai_memo: dict):
    """Insert AI-generated narrative sections into the Word document."""
    _heading(doc, "AI-Generated Investment Narrative", 1)

    p = doc.add_paragraph()
    r = p.add_run(
        "The following narrative was generated by Claude (claude-sonnet-4-6) using only the "
        "verified deal data and live market API results above. No figures were fabricated."
    )
    r.font.italic = True
    r.font.size = Pt(9)
    ...
    for key, label in section_labels:
        ...
        # Render bullet lines (starting with •) as list items, rest as normal paragraphs
...
def generate_word(pro_forma: dict, market_data: dict, job_id: str,
    ...
    # ===== AI-GENERATED NARRATIVE (inserted before data sections) =====
    has_ai_memo = ai_memo and not ai_memo.get("error") and ai_memo.get("executive_summary")
    if has_ai_memo:
        _add_ai_memo_section(doc, ai_memo)
    ...
    # Unit mix table (if provided)
    has_unit_mix = unit_mix and unit_mix.get("units")
    if has_unit_mix:
        doc.add_paragraph()
        _heading(doc, "Unit Mix", 2)
        um_headers = ["Type", "Count", "% of Total", "In-Place Rent", "Market Rent", "Annual Revenue"]
        has_hud = any(u.get("hud_fmr") for u in unit_mix["units"])
        if has_hud:
            um_headers += ["HUD FMR", "vs. HUD"]
        um_rows = []
        total_units = unit_mix["total_units"]
        for u in unit_mix["units"]:
            pct_of_total = f"{u['count'] / total_units * 100:.0f}%" if total_units else "0%"
            mkt = _cur(u["market_rent"]) if u.get("market_rent", 0) > 0 else "N/A"
            row = [u["type_name"], str(u["count"]), pct_of_total,
                   _cur(u["in_place_rent"]), mkt, _cur(u["annual_revenue"])]
            if has_hud:
                row.append(_cur(u.get("hud_fmr")))
                vs = u.get("vs_hud_pct")
                row.append(f"{vs:+.1f}%" if vs is not None else "N/A")
            um_rows.append(row)
        # Totals row
        blended = unit_mix["blended_in_place_rent"]
        total_rev = unit_mix["total_annual_revenue"]
        totals_row = ["TOTAL", str(total_units), "100%", f"${blended:,.0f} (blended)",
                      "", _cur(total_rev)]
        if has_hud:
            totals_row += ["", ""]
        um_rows.append(totals_row)
        _table(doc, um_headers, um_rows)

        if unit_mix.get("total_rent_premium_potential", 0) > 0:
            doc.add_paragraph()
            doc.add_paragraph(
                f"Value-add opportunity: ${unit_mix['total_rent_premium_potential']:,.0f}/year total rent "
                f"upside if all units reach market rate "
                f"(blended ${unit_mix['blended_in_place_rent']:,.0f} → ${unit_mix['blended_market_rent']:,.0f}/unit/month)."
            )
